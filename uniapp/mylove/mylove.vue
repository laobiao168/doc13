		...
		<view style="background: #f7f7f7;">
			<view class="inner98">
				<view class="grtopnav">
					...
					<view class="item" :class="{cur:lotteryType==2}" @click="changeLottery(2)">澳彩</view>
					...
					<view class="item" :class="{cur:lotteryType==4}" @click="changeLottery(4)">新彩</view>
				</view>
				<view class="tab">
			...
			<view class="item" v-for="(item,index) in list" :key="index" @click="godetail(item)">
				<view class="inner98">
					<view style="display: flex; justify-content: space-between;">
						...
						<view class="dzsctxt" style="margin: 0;">
	...
	export default {
		...
		methods:{
			...
			getlist(){
				if(this.tab==1){
					//高手论坛
					this.$request('user/mylovelist', {tab:this.tab, page:this.page, keyword:this.keyword, type:1, lotteryType:this.lotteryType, order:4}).then((res)=>{
						console.log(res);
					...
					//发现
					this.$request('user/mylovelist', {tab:this.tab, page:this.page, keyword:this.keyword, type:3, lotteryType:this.lotteryType, order:4}).then((res)=>{
						console.log(res);
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
					...
					//资料大全
					this.$request('user/mylovelist', {tab:this.tab, page:this.page, keyword:this.keyword, lotteryType:this.lotteryType}).then((res)=>{
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
							this.list = [];
					...
					//图解
					this.$request('user/mylovelist', {tab:this.tab, page:this.page, keyword:this.keyword, type:2, lotteryType:this.lotteryType, order:4}).then((res)=>{
						console.log(res);
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
					...
					//图库
					this.$request('user/mylovelist', {page:this.page, keyword:this.keyword,lotteryType:this.lotteryType, tab:this.tab}).then((res)=>{
						console.log(res);
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
...
.tab .item{color: #999; line-height: 40px; padding: 5px 0; text-align: center;}
.tab .item.cur{color: #07c160;}

.alist .item  .title .wz{color: #444; font-size: 1rem; letter-spacing: 1; margin-top: 0px; font-weight: 200;}
</style>
