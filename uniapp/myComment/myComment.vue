	...
	export default {
		...
		methods:{
			...
			getlist(){
				if(this.tab==1){
					//高手论坛
					this.$request('user/mycommentlist', {tab:this.tab, page:this.page, keyword:this.keyword, type:1, lotteryType:this.lotteryType, order:4}).then((res)=>{
						console.log(res);
					...
					//发现
					this.$request('user/mycommentlist', {tab:this.tab, page:this.page, keyword:this.keyword, type:3, lotteryType:this.lotteryType, order:4}).then((res)=>{
						console.log(res);
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
					...
					//资料大全
					this.$request('user/mycommentlist', {tab:this.tab, page:this.page, keyword:this.keyword, lotteryType:this.lotteryType}).then((res)=>{
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
							this.list = [];
					...
					//图解
					this.$request('user/mycommentlist', {tab:this.tab, page:this.page, keyword:this.keyword, type:2, lotteryType:this.lotteryType, order:4}).then((res)=>{
						console.log(res);
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
					...
					//图库
					this.$request('user/mycommentlist', {page:this.page, keyword:this.keyword,lotteryType:this.lotteryType, tab:this.tab}).then((res)=>{
						console.log(res);
						if(res.data.length>0){
							this.list = this.list.concat(res.data);
						}else{
...
.item .pl{background-color: #f7f7f7; font-size: .7rem; color: #666; padding: 10px; margin-top: 10px;}
</style>
